...
class AnomalyDetail extends StatefulWidget {
  IncidentListViewModel? entity;
  AnomalyDetail(this.entity, {super.key});

  @override
  State<AnomalyDetail> createState() => _AnomalyDetailState();
}

class _AnomalyDetailState extends State<AnomalyDetail> {
  ...
  void initState() {
    ...
    print('播放-${Config.realTimeVideoIP}/${widget.entity!.cameraId}-ai/livestream/index.m3u8');
    videoBloc.add(VideoLoadEvent('${Config.realTimeVideoIP}/${widget.entity!.cameraId}-ai/livestream/index.m3u8', isLive: true));
    super.initState();
  }


  Future<void> _showEditDialog(IncidentListViewModel entity) async {
    await showDialog(
          ...
          child: Column(
            ...
            children: [
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text(widget.entity?.locationName ?? ''),
                  Row(
                    children: [
                      const Text('攝影機IP'),
                      const SizedBox(
                      ...
                      context.read<LoginBloc>().state.userEntity!.permission == 0
                          ...
                          : GestureDetector(
                              onTap: () {
                                ...
                                print('videoUrl---cameraId:${widget.entity!.cameraId}--${widget.entity?.videoUrl}----${parts[1]}');
                                String fireUrl = '${Config.recordsVideoIP}/api/mp4?cameraId=${widget.entity!.cameraId}-ai&hlsFile=${parts[1]}';
